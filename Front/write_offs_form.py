...
class WriteOffsForm(ctk.CTkFrame):
    def __init__(self, master, window_w, window_h):
        ...
        self.__creating_frame = self.__create_creation_frame(
            article_entry_w=round(50 * table_width_percentage),
            count_entry_w=round(20 * table_width_percentage),
            write_off_reason_entry_w=round(30 * table_width_percentage),
        ...
        self.__write_offs_table = CTkTable.CTkTable(
            master=self.__table_frame,
            values=[
                ["Дата и время операции", "Артикул товара", "Кол-во", "Причина списания"]],
            row=12,
            column=4,
        ...
        self.__write_offs_table.edit_row(0, font=("Arial", font_size))
        self.__write_offs_table.edit_column(0, width=round(30 * table_width_percentage))
        self.__write_offs_table.edit_column(1, width=round(20 * table_width_percentage))
        self.__write_offs_table.edit_column(2, width=round(20 * table_width_percentage))
        self.__write_offs_table.edit_column(3, width=round(30 * table_width_percentage))
        ...
        self.__write_offs_table.grid(row=1, column=0, padx=3, pady=3)
    ...
    def __create_creation_frame(
        ...
        self.__count_entry.grid(row=1, column=1, padx=2)
        self.__write_offs_reason_entry.grid(row=1, column=2, padx=2)
