...
class NomenclatureForm(ctk.CTkFrame):
    ...
    def __add_product(self):
        ...
        try:
            ...
            added_record = [article, name, f"{round(float(buy_price), 2):.2f}", f"{round(float(sel_price), 2):.2f}", prod_type, prod_unit, 0]
    ...
    def __update_product(self):
        ...
        if dialog.modal_result:
            ...
            try:
                ...
                updated_record = [article, name, f"{round(float(buy_price), 2):.2f}", f"{round(float(sel_price), 2):.2f}", prod_type, prod_unit, count]
